...
#   - 凭据由平台执行器通过受控环境变量注入（DEEPSEEK / DASHSCOPE / ARK / VOLC）
...
DH = _resolve_dh()
VENV_PY = os.environ.get('DSH_TTS_PYTHON', '') or os.path.join(DH, 'venv', 'Scripts', 'python.exe')
TTS_SCRIPT = os.environ.get('DSH_TTS_SCRIPT', '') or os.path.join(DH, 'tts.py')

# 平台不读取运行账户 HOME 目录中的凭据文件；如需兼容文件形式，必须显式传入此路径。
CRED_FILE = os.environ.get('DSH_CREDENTIALS_FILE', '')
...
def call_deepseek(prompt, json_mode=True, max_tokens=4000):
    ...
    if not key:
        return None
    base_url = os.environ.get('DSH_DEEPSEEK_BASE_URL', 'https://api.deepseek.com').rstrip('/')
    body = {
    ...
    data = http_json(base_url + '/chat/completions', body,
