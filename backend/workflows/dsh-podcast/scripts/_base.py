...
def make_waveform(audio, out_mp4, duration, size=None, fg='0x67e8f9', bg='0x101828', fps=25):
    ...
    fc = ('[0:a]showwaves=s=%dx%d:mode=cline:rate=%d:colors=%s[wave];'
          'color=c=%s:s=%dx%d:d=%.3f:r=%d[bg];[bg][wave]overlay=x=0:y=0:shortest=1[out]'
          % (w, h, fps, fg, bg, w, h, duration, fps))
    try:
        run_ff(['-y', '-i', audio, '-filter_complex', fc, '-map', '[out]',
                '-c:v', 'libx264', '-pix_fmt', 'yuv420p', '-crf', '20', '-movflags', '+faststart', out_mp4])
    except Exception as exc:
        # Remotion's bundled FFmpeg omits showwaves. Keep the podcast usable
        # by falling back to a stable canvas; subtitles and the real audio
        # are still added in later stages.
        print('[waveform] showwaves 不可用，使用静态播客画布：%s' % exc)
        canvas = os.path.join(BASE, 'assets', 'podcast-canvas.png')
        if os.path.isfile(canvas):
            run_ff(['-y', '-loop', '1', '-i', canvas, '-t', '%.3f' % duration,
                    '-vf', 'scale=%d:%d' % (w, h),
                    '-an', '-c:v', 'libx264', '-pix_fmt', 'yuv420p', '-crf', '20',
                    '-movflags', '+faststart', out_mp4])
        else:
            raise RuntimeError('FFmpeg 缺少 showwaves，且未找到静态播客画布。') from exc
    return out_mp4
