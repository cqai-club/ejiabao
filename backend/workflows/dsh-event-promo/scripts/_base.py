...
def normalize_clip(src, out_mp4, size, fps=25):
    ...
    try:
        ...
        print('[normalize] 归一化滤镜不可用，直接复制分段：%s' % exc)
...
def ken_burns(image, out_mp4, duration, size=None, fps=25, mode='in'):
    ...
    try:
        ...
        # Remotion's bundled FFmpeg omits zoompan. Keep the segment usable
        # with a static poster scaled to the requested canvas.
        print('[ken-burns] zoompan 不可用，使用静态海报：%s' % exc)
        run_ff(['-y', '-loop', '1', '-i', image, '-vf', 'scale=%d:%d' % (w, h), '-t', '%.2f' % duration,
                '-c:v', 'libx264', '-pix_fmt', 'yuv420p', '-crf', '20', '-movflags', '+faststart', out_mp4])
    return out_mp4
